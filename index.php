<!DOCTYPE html>
<html>
<head>

<!-- Google tag (gtag.js) -->
<script async src="https://www.googletagmanager.com/gtag/js?id=G-5N8YJMK5MV"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'G-5N8YJMK5MV');
</script>

    <meta name="viewport" content="width=device-width, user-scalable=no">
    <link href="style.css" rel="stylesheet">
    <link href="icon.png" rel="icon">
    <title>Фонд SCP</title>
</head>
<body>
<!-- Google Tag Manager (noscript) -->
<noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-N5JMXT5W"
height="0" width="0" style="display:none;visibility:hidden"></iframe></noscript>
<!-- End Google Tag Manager (noscript) -->
    <header>
        <a href="index.html">
            <img src="shapka.png" id="banner">
        </a>
    </header>
    <main id="main">
        <nav>
            <div class="addendum">
                <h2>Информация</h2>
                <ul>
                    <li><a href="info.html">О Фонде</a></li>
                    <li><a href="object-classes.html">Классы объектов</a></li>
                    <li><a href="access-levels.html">Уровни допуска</a></li>
                    <li><a href="personnel-classes.html">Классы персонала</a></li>
                    <li><a href="services.html">Внутренние службы</a></li>
                    <li><a href="sites.html">Учреждения Фонда</a></li>
                    <li><a href="organizations.html">Связанные организации</a></li>
                </ul>
            </div>
            <div class="addendum">
                <h2>Объекты</h2>
                <ul>
                    <li><a href="objects.html">Объекты SCP-SP</a></li>
                    <li><a href="original-objects.html">Объекты SCP</a></li>
                </ul>
            </div>
        </nav>
        <article>
            <div class="warning">
                <p><strong>ВНИМАНИЕ</strong></p>
                <p>ПРЕДСТАВЛЕННЫЕ ДАЛЕЕ МАТЕРИАЛЫ</p>
                <p><strong>СТРОГО ЗАСЕКРЕЧЕНЫ</strong></p>
                <p>ВЕДЕТСЯ НАБЛЮДЕНИЕ. НАРУШЕНИЯ РЕЖИМА СЕКРЕТНОСТИ ОТСЛЕЖИВАЮТСЯ И КАРАЮТСЯ. ДРУГИХ ПРЕДУПРЕЖДЕНИЙ НЕ БУДЕТ.</p>
            </div>
            <div class ="daily">
              <p>ОБЪЕКТ ДНЯ:</p>
              <?php include 'daily_article.php'; ?>
            </div>
            <div class="addendum">
                <h2>2024/11/18</h2>
                <img src="research.png" class="image">
                <p><i>Научная служба Фонда SCP приглашает Вас присоединиться к исследованию явлений, бросающих вызов привычным представлениям о реальности. - Д-р Роханов</i></p>
                <h2>Смотреть также</h2>
                <ul>
                    <li><a href="services.html#scientific">Научная служба</a></li>
                    <li><a href="services.html#medical">Медицинская служба</a></li>
                </ul>
            </div>
            <div class="warning public">
                <p><strong>ОШИБКА ИДЕНТИФИКАЦИИ</strong></p>
                <p>ИНФОРМАЦИЯ ОГРАНИЧЕНА</p>
            </div>
            <a href="https://discord.gg/X2q9zTvUkV" class="button public">
                <p>Войти в систему</p>
            </a>
        </article>
    </main>
    <script src="script.js"></script>
</body>
</html>
