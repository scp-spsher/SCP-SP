...
const Layout: React.FC<LayoutProps> = ({ 
  ...
  // Use the SIMULATED clearance for UI decisions
  ...
  // Theme colors based on simulation
  ...
  const logoColor = isO5View ? 'text-yellow-500 border-yellow-500' : 'text-scp-text border-scp-text';
      ...
      <aside className={`hidden md:flex flex-col w-64 border-r ${borderColor} bg-scp-panel z-10 transition-all duration-500`}>
        ...
        <nav className="flex-1 overflow-y-auto py-4">
          <ul className="space-y-1">
            {navItems.map((item) => {
              // Check access based on SIMULATED clearance
        ...
        <div className={`p-4 border-t ${borderColor}`}>
          <div className="flex items-center justify-between text-xs text-gray-500 mb-4">
            <span>ДОСТУП:</span>
            <span className={`${isO5View ? 'text-yellow-500' : 'text-scp-accent'} font-bold`}>
              {simulatedClearance === 6 ? 'OMNI' : `УРОВЕНЬ ${simulatedClearance}`}
            </span>
          </div>
          <button 
            onClick={onLogout}
          ...
          <div className="md:hidden absolute top-16 left-0 w-full h-[calc(100%-4rem)] bg-scp-dark z-30 border-b border-gray-800 p-4">
             ...
             {isSuperAdmin && (
              <div className="mb-4 pb-4 border-b border-gray-800">
                <select 
                  value={simulatedClearance}
                  onChange={(e) => setSimulatedClearance(Number(e.target.value))}
                  className="w-full bg-black border border-gray-700 text-xs text-scp-terminal p-3"
                >
                  <option value={6}>ВИД: OMNI</option>
                  <option value={5}>ВИД: УРОВЕНЬ 5</option>
                  <option value={4}>ВИД: УРОВЕНЬ 4</option>
                  <option value={3}>ВИД: УРОВЕНЬ 3</option>
                  <option value={2}>ВИД: УРОВЕНЬ 2</option>
                  <option value={1}>ВИД: УРОВЕНЬ 1</option>
                </select>
              </div>
             )}
            ...
            {navItems.map((item) => {
                ...
                <li key={item.id}>
                  {hasAccess ? (
                    <button
                      ...
                      className={`w-full flex items-center px-4 py-4 text-sm border-l-2 ${
                        currentPage === item.id 
                          ? activeBg
             ...
             <li className="pt-4 mt-4 border-t border-gray-800">
               <button 
                onClick={onLogout}
                className="w-full flex items-center px-4 py-4 text-sm text-red-500 hover:bg-gray-900"
              >
                <LogOut size={18} className="mr-3" /> ЗАВЕРШИТЬ СЕССИЮ
              </button>
             </li>
          </ul>
          </div>
